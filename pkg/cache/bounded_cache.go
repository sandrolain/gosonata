...
// CacheStats holds runtime statistics for a BoundedCache.
...
// BoundedCache is a thread-safe, FIFO-evicting expression cache with lock-free reads.
//
...
//
// BoundedCache implements the Cacher interface.
...
// New creates a new BoundedCache with the given capacity.
// If capacity <= 0, a default of 256 is used.
func New(capacity int) *BoundedCache {
	if capacity <= 0 {
		capacity = 256
	}
	b := &BoundedCache{capacity: capacity}
...
func (b *BoundedCache) Get(key string) (*types.Expression, bool) {
...
func (b *BoundedCache) Set(key string, expr *types.Expression) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *BoundedCache) GetOrCompile(key string, compile func() (*types.Expression, error)) (*types.Expression, error) {
...
// Lock-free.
func (b *BoundedCache) Stats() CacheStats {
	return CacheStats{
		Hits:      b.hits.Load(),
